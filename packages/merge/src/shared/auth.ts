...
/**
 * Detect auth mode from environment variables and return Octokit constructor options.
 *
...
export function getAuthOptions(): ConstructorParameters<typeof Octokit>[0] {
  const appId =
    process.env.FLEET_APP_ID || process.env.GITHUB_APP_ID;
  const installationId =
    process.env.FLEET_APP_INSTALLATION_ID ||
...
export function createMergeOctokit(): Octokit {
  return new Octokit(getAuthOptions());
}
