...
import { JulesRateLimitError, JulesApiError } from '../src/errors.js';
...
describe('ApiClient Retry Logic', () => {
  ...
  beforeEach(() => {
    ...
    // Mock Math.random to avoid jitter in tests (0.5 results in 0 jitter)
  ...
  describe('429 Rate Limit Retry', () => {
    it('should retry once after 1s delay if 429 is returned once', async () => {
      ...
      // 2. Advance time by 999ms. Retry should NOT happen yet (assuming 1000ms base delay).
  ...
  describe('Server Error Retry (5xx)', () => {
    it.each([500, 502, 503, 504])('should retry on status %i', async (status) => {
      const apiClient = new ApiClient({
        apiKey: 'test-key',
        baseUrl,
        requestTimeoutMs: 1000,
    ...
    it('should eventually throw JulesApiError (not RateLimitError) if 5xx persists', async () => {
      const apiClient = new ApiClient({
        apiKey: 'test-key',
        baseUrl,
        requestTimeoutMs: 1000,
      ...
      const promise = apiClient.request('test');
      const failureExpectation = expect(promise).rejects.toThrow(JulesApiError);
      ...
      // Wait 2s -> Attempt 3 (elapsed 3s > maxRetryTimeMs 2s)
      // Should fail before attempt 3, or rather attempt 3 is blocked by time check.
      await vi.advanceTimersByTimeAsync(2000);

      // We expect it to stop retrying.
      // Depending on implementation, it might check time before waiting or before call.
      // My implementation checks time before waiting.
      // So at t=1s, elapsed=1s < 2s. Waits 2s.
      // At t=3s, loop continues.
      // Next iteration: checks time. elapsed=3s > 2s.
      // Should throw.
